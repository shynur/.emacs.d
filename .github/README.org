...
Latest release, using *GUI*, run as *daemon-clients* on same machine ([[./shynur/Emacs-use_daemon.md][如何运行 Emacs 后台服务?]]).
...
- [ ] 拖动 GUI 时自动缩小应用窗口.
- [ ] (_shynur/bug#1_) =C-h v= 后按 =TAB= 补全时, 过滤掉 ~prefix--*~ 和 ~*-internal~.
